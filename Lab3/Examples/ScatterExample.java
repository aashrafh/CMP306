...
public class ScatterExample {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException, ClassNotFoundException {
		...
		if (rank == root) {
			...
			// Scatter in the root processes uses the sendbuff to send data to all
			// processes, and
			// recvbuffer to receive the data sent to it
			// Here two integer is sent to each process, and two integer is received from
			// itself, however if we don't want to use the receive buffer we can set it to
			// dummy.
			...
			int[] dummy = new int[8]; // Although it is not needed, it needs to match the total number of sent
										// messages
